"""
Plugin entry point for IDS System.
...
from .ids import analyze_pcap_file, live_monitor, IdsResult

logger = get_logger("hacks.ids_system")

PLUGIN_METADATA = {
    "id": "hacks.ids_system",
    "name": "IDS System",
    "category": "hacks",
    "description": "Detects ARP spoofing, plaintext credentials, DNS tunneling, port scans, and other intrusion signatures.",
    "icon": "shield-alert",
}

SEVERITY_COLOR = {"critical": DANGER, "high": DANGER, "medium": WARN, "low": MUTED, "info": ACCENT}


class IdsSystemWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        title = QLabel("IDS System")
        title.setStyleSheet(f"font-size: 20px; font-weight: 700; color:{TEXT};")
        subtitle = QLabel(
            "Signature and anomaly-based intrusion detection: ARP spoofing, known "
            "malicious/backdoor ports, plaintext credentials in transit, DNS tunneling "
            "indicators, port scans, and statistical traffic outliers."
        )
        ...
        self.table.setHorizontalHeaderLabels(["Signature", "Source", "Detail"])
    ...
    def on_choose_file(self):
        ...
        self.status_message("Scanning capture file for intrusion signatures...")
    ...
    def on_live_result(self, result: IdsResult):
    ...
    def on_result(self, result: IdsResult):
        ...
        if result.alerts:
            self.summary_label.setText(
                ...
                f"⚠ {len(result.alerts)} alert(s) triggered."
            )
        else:
            self.summary_label.setText(
                f"{result.total_packets} packets over {result.duration_seconds}s — "
                f"no intrusion signatures detected."
            )

        self.table.setRowCount(len(result.alerts))
        for row, alert in enumerate(result.alerts):
            sig_item = QTableWidgetItem(f"{alert.severity.upper()} — {alert.signature}")
            sig_item.setForeground(QColor(SEVERITY_COLOR.get(alert.severity, MUTED)))
            self.table.setItem(row, 0, sig_item)
            self.table.setItem(row, 1, QTableWidgetItem(alert.source or "—"))
            self.table.setItem(row, 2, QTableWidgetItem(alert.detail))

        self._publish_events(result)

    def on_export(self):
        path = export_table_to_csv(self, self.table, "ids_alerts.csv")
        if path:
            self.summary_label.setText(f"Exported to {path}")

    def _publish_events(self, result: IdsResult):
        if result.alerts:
            worst = result.alerts[0]  # already sorted by severity
            self.event_bus.publish(SecurityEvent(
                source="hacks.ids_system",
                category="hacks",
                title=f"{len(result.alerts)} IDS alert(s) — worst: {worst.signature}"
                      + (f" from {worst.source}" if worst.source else ""),
                severity=worst.severity,
                details={"alert_count": len(result.alerts), "total_packets": result.total_packets},
            ))
        else:
            self.event_bus.publish(SecurityEvent(
                source="hacks.ids_system",
                category="hacks",
                title=f"IDS scan of {result.total_packets} packet(s) — no signatures matched",
                severity="info",
                details={"total_packets": result.total_packets},
            ))
        logger.info(f"IDS analysis complete: {len(result.alerts)} alert(s)")


def get_widget(event_bus):
    return IdsSystemWidget(event_bus)
